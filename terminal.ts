import { Disposable, onSignal } from "https://deno.land/std/signal/mod.ts";
...
type Writer = Deno.Writer & Deno.WriterSync & { rid: number };
...
export class Terminal
{
    ...
    private w: number = 0;
    private h: number = 0;
    private disposeSignal: Disposable|null = null;
    ...
    {
        if ( this.disposeSignal ) { this.disposeSignal.dispose(); }
    }

    protected enc( str: string ) { return this.encoder.encode( str ); }
    ...
    private updateSize()
    {
        const size = Deno.consoleSize( this.writer.rid );
        this.w = size.columns;
        this.h = size.rows;
    }

    public get width() { return this.w; }
    public get columns() { return this.w; }
    public get height() { return this.h; }
    public get rows() { return this.h; }

    public write( buf: Uint8Array ) { return this.syncMode ? this.writeSync( buf ) : this.writeAsync( buf ); }
    public writeSync( buf: Uint8Array ) { return this.writer.writeSync( buf ); }
